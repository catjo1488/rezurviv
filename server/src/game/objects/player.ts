...
leprechaunInvincibleTicker = 0;
    ...
    update(dt: number): void {
        ...
        if (this.hasPerk("woods")) {
            this.health += PerkProperties.woods.healthRegen * dt;
        }
        ...
        const shouldSendEmote = (emote: Emote) => {
            ...
            if (emotePlayer) {
                ...
                if (
                 ...
                 if (
                    (emotePlayer.role === "admiral" || emotePlayer.role === "captain") &&
                    emotePlayer.teamId === player.teamId
                ) {
                    return true;
                }
                 if (
                    (emotePlayer.role === "Vice_Admiral" || emotePlayer.role === "captain") &&
                    emotePlayer.teamId === player.teamId
                ) {
                    return true;
                }
        ...
        if (
            ...
            if (chest && !isHeadShot) {
                ...
                if (this.hasPerk("armor_master")) {
                    chestReduction *= 1 + (PerkProperties.armor_master?.armorBonus ?? 2);
                }
            ...
            if (helmet) {
                let helmetReduction = helmet.damageReduction * (isHeadShot ? 1 : 0.5);
                if (this.hasPerk("armor_master")) {
...
if (killCreditSource !== this && (killCreditSource.teamId !== this.teamId || attackerIsDeserter || targetIsDeserter)) {
    ...
    recalculateSpeed(hasTreeClimbing: boolean): void {
        ...
        if (this.lowHpSurgeTicker > 0 && this.hasPerk("low_hp_surge")) {
            const mult = (PerkProperties.low_hp_surge as any)?.speedMult ?? 2.5;
            if (mult && mult > 0) this.speed *= mult;
        }
